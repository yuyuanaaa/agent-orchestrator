...
public class CommonUtils {
    ...
    /**
     * 启动时初始化内置知识库（FAQ + 菜品口味 PDF）并落盘快照。
     * <p>
     * 幂等处理：向量库快照已存在（说明此前已写入内置知识 + 用户数据）时跳过，
     * 避免每次重启重复写入内置知识导致检索结果重复。
     */
    @PostConstruct
    public void init(){
        File snapshot = new File(FileConstant.VECTOR_STORE_SNAPSHOT);
        if (snapshot.exists() && snapshot.length() > 0) {
            log.info("向量库快照已存在，跳过内置知识初始化");
            return;
        }
        log.info("首次启动，写入内置知识库到向量库");
    ...
    /**
     * 将向量库序列化落盘，重启后由 {@link com.agentorchestrator.platform.AgentPlatformApplication#vectorStore}
     * 加载恢复。关闭时与定时任务均会触发，兼顾优雅停机与异常崩溃两种场景。
     */
    public void persist(){
        try {
            File snapshot = new File(FileConstant.VECTOR_STORE_SNAPSHOT);
            File dir = snapshot.getParentFile();
            if (dir != null && !dir.exists()) {
                dir.mkdirs();
            }
            vectorStore.save(snapshot);
        } catch (Exception e) {
            log.warn("向量库快照持久化失败", e);
        }
    }

    /** 优雅停机时持久化一次，保证正常重启不丢向量数据 */
    @PreDestroy
    public void persistOnShutdown(){
        persist();
    }

    /** 定时持久化，兜底异常崩溃场景（最多丢失最近 60 秒内的向量写入） */
    @Scheduled(fixedDelay = 60000, initialDelay = 60000)
    public void persistPeriodically(){
        persist();
    }
