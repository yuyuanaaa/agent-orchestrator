package com.agentorchestrator.platform.entity.querys;
...
public class OrderQuery {
    ...
    @ToolParam(required = false,description = "订单所含菜品名称，和其所对应的单价（已废弃：单价由服务端从数据库读取并重算，无需填写）")
    private Map<String, BigDecimal> dishesPrice;

    @ToolParam(required = false,description = "订单所含套餐名称，和其所对应的单价（已废弃：单价由服务端从数据库读取并重算，无需填写）")
