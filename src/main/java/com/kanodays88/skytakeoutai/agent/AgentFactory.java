...
public class AgentFactory {
    ...
    public RouterAgent createRouterAgent(SseEmitter emitter) throws IOException {
        return new RouterAgent(chatModel, vectorStore, allTools, skillRegistry, emitter);
    }

    /** 创建简单对话智能体（每个 SSE 会话一个实例） */
    public SimpleChatAgent createSimpleChatAgent() throws IOException {
        return new SimpleChatAgent(chatModel, allTools);
    }

    /** 创建任务执行智能体，按子任务契约只挂载所需工具 */
    public Kanodays88Manus createManus(ToolCallback[] tools) {
        return new Kanodays88Manus(tools, chatModel);
    }
}
