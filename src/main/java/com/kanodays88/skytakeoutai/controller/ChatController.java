...
import com.kanodays88.skytakeoutai.agent.AgentFactory;
...
import com.kanodays88.skytakeoutai.utils.DirectoryCleaner;
...
public class ChatController {
    ...
    public Result<String[]> historyQuery() {
        return Result.success(FileScanUtils.getFilenamesWithoutExtension(chatMemoryDir()));
    }
    ...
    private FileBasedChatMemory chatMemory() throws IOException {
        return new FileBasedChatMemory(chatMemoryDir());
    }

    private String chatMemoryDir() {
        return Paths.get(FileConstant.FILE_SAVE_DIR, currentUserName(), "chatMemory").toString();
    }
    ...
    private interface SseTask {
        String execute(SseEmitter emitter, FileBasedChatMemory memory) throws Exception;
    }
